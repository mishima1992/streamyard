...
export const generateSsoToken = async (req, res) => {
  try {
    ...
    user.ssoToken = crypto.createHash('sha256').update(ssoToken).digest('hex');
    user.ssoTokenExpires = Date.now() + 60 * 1000;
    await user.save();
    res.json({ ssoToken });
  } catch (error) {
...
export const verifySsoToken = async (req, res) => {
  try {
    ...
    const hashedToken = crypto.createHash('sha256').update(ssoToken).digest('hex');
    const user = await User.findOne({
      ssoToken: hashedToken,
      ssoTokenExpires: { $gt: Date.now() },
    });
    ...
    user.ssoTokenExpires = undefined;
    await user.save();
